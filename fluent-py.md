...
---
  ...
  * [2. 序列构成的数组](#2-序列构成的数组)
     ...
     * [2.9 当列表不是首选时](#29-当列表不是首选时)
        * [2.9.1 数组](#291-数组)
        * [2.9.2 双向队列和其他形式的队列](#292-双向队列和其他形式的队列)
  * [3. 字典和集合](#3-字典和集合)
     * [3.1 字典创建方法](#31-字典创建方法)
     * [3.2 常见的映射方法](#32-常见的映射方法)
     * [3.3 用 setdefault 处理找不到的键](#33-用-setdefault-处理找不到的键)
     * [3.4 映射的弹性键查询](#34-映射的弹性键查询)
        * [3.4.1 defaultdict：处理找不到的键的一个选择](#341-defaultdict处理找不到的键的一个选择)
        * [3.4.2 特殊方法 __missing__](#342-特殊方法-__missing__)
     * [3.5 字典的变种](#35-字典的变种)
     * [3.6 子类化 UserDict](#36-子类化-userdict)
     * [3.7 不可变映射序列](#37-不可变映射序列)
     * [3.8 集合论](#38-集合论)
     * [3.8.1 集合字面量](#381-集合字面量)
        * [3.8.2 集合推导](#382-集合推导)
     * [3.8.3 集合的操作](#383-集合的操作)
     * [3.9 dict 和 set 的背后](#39-dict-和-set-的背后)
        * [3.9.1 字典中的散列表](#391-字典中的散列表)
        * [3.9.2 dict 的实现及其导致的结果](#392-dict-的实现及其导致的结果)
        * [3.9.3 set 的实现以及导致的结果](#393-set-的实现以及导致的结果)
  * [4. 文本和字节序列](#4-文本和字节序列)
     * [处理文本文件](#处理文本文件)
  * [5. 一等对象](#5-一等对象)
     * [5.1 高阶函数](#51-高阶函数)
     * [5.2 匿名函数](#52-匿名函数)
     * [5.3 可调用对象](#53-可调用对象)
     * [5.4 用户定义的可调用类型](#54-用户定义的可调用类型)
     * [5.5 函数内省](#55-函数内省)
     * [5.6 从定位参数到仅限关键字参数](#56-从定位参数到仅限关键字参数)
     * [5.7 函数注解](#57-函数注解)
     * [5.8 支持函数式编程的包](#58-支持函数式编程的包)
        * [5.8.1 operator 模块](#581-operator-模块)
        * [5.8.2 使用 functools.partial 冻结参数](#582-使用-functoolspartial-冻结参数)
  * [6. 使用一等函数实现策略模式](#6-使用一等函数实现策略模式)
  * [6.1　案例分析：重构“策略”模式](#61案例分析重构策略模式)
     * [6.1.1　经典的“策略”模式](#611经典的策略模式)
     * [6.1.2　使用函数实现“策略”模式](#612使用函数实现策略模式)
     * [6.1.3 选择最佳策略：简单的方式](#613-选择最佳策略简单的方式)
     * [6.1.4　找出模块中的全部策略](#614找出模块中的全部策略)
  * [6.2 “命令”模式](#62-命令模式)
  * [7. 函数装饰器和闭包](#7-函数装饰器和闭包)
     * [7.1　装饰器基础知识](#71装饰器基础知识)
     * [7.2 使用装饰器改进“策略”模式](#72-使用装饰器改进策略模式)
     * [7.3 变量作用域规则](#73-变量作用域规则)
     * [7.4 闭包](#74-闭包)
     * [7.5 nonlocal 声明](#75-nonlocal-声明)
     * [7.6 实现一个简单的装饰器](#76-实现一个简单的装饰器)
     * [7.7 标准库中的装饰器](#77-标准库中的装饰器)
        * [7.7.1 使用 functools.lru_cache 做备忘](#771-使用-functoolslru_cache-做备忘)
        * [7.7.2 单分派泛函数](#772-单分派泛函数)
     * [7.8 叠放装饰器](#78-叠放装饰器)
     * [7.9 参数化装饰器](#79-参数化装饰器)
  * [8. 对象引用、可变性和垃圾回收](#8-对象引用可变性和垃圾回收)
     * [8.1 变量不是盒子](#81-变量不是盒子)
     * [8.2 标识、相等性和别名](#82-标识相等性和别名)
        * [8.2.1 在==和is之间选择](#821-在和is之间选择)
        * [8.2.2 元组的相对不可变性](#822-元组的相对不可变性)
     * [8.3 默认做浅复制](#83-默认做浅复制)
     * [8.4 函数的参数作为引用时](#84-函数的参数作为引用时)
        * [8.4.1 不要使用可变类型作为参数的默认值](#841-不要使用可变类型作为参数的默认值)
        * [8.4.2 防御可变参数](#842-防御可变参数)
     * [8.5 del 和 垃圾回收](#85-del-和-垃圾回收)
     * [8.6 弱引用](#86-弱引用)
        * [8.6.1 WeakValueDictionary](#861-weakvaluedictionary)
        * [8.6.2 弱引用的局限](#862-弱引用的局限)
     * [8.7 Python 对不可变类型施加的把戏](#87-python-对不可变类型施加的把戏)
     * [8.8 Summary](#88-summary)
  * [9. 符合 Python 风格的对象](#9-符合-python-风格的对象)
     * [9.1 对象的表示形式](#91-对象的表示形式)
     * [9.2 再谈向量类](#92-再谈向量类)
     * [9.3 备选构造方法](#93-备选构造方法)
     * [9.4 classmethod 与 staticmethod](#94-classmethod-与-staticmethod)
     * [9.5 格式化显示](#95-格式化显示)
     * [9.6 可散列的 Vector2d](#96-可散列的-vector2d)
     * [9.7 使用 __slots__ 类属性节省空间](#97-使用-__slots__-类属性节省空间)
  * [10 序列的修改、散列和切片](#10-序列的修改散列和切片)
     * [10.1 能处理切片的 __getitem__ 方法](#101-能处理切片的-__getitem__-方法)
     * [10.2 动态存取特性](#102-动态存取特性)
     * [10.3 散列和快速等值测试](#103-散列和快速等值测试)
     * [10.4 格式化](#104-格式化)
  * [11 接口：从协议到抽象基类](#11-接口从协议到抽象基类)
  * [14 可迭代的对象、迭代器和生成器](#14-可迭代的对象迭代器和生成器)
